...
import {Log, LogInsertProperty} from "../../src/ms/lc";
import {IRes, IResType} from "../../src/request";
...
describe('log',()=>{
    ...
    test('借还记录增加', async() => {
        const res = await lc.log.insert(json) as IResType<Log>
        // console.log(res)
    ...
    test('借还工具使用率统计', async() => {
        const res = await lc.logProcessStatistics({statisticsType:"Log",chartType:"Form",companyId:12000,pageSize:10,curPageNo:1})
        console.log(res)
        // expect(res.data[0].deviceId).toBe(deviceId)
    },2000)
})
