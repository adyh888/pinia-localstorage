...
describe('role_access',()=>{
    ...
    test('app getAccessByRole', async() => {
        const mc = Mc(service)
        const res = await mc.getAccessByRole(8)
        console.log(res)
    ...
    test('app tree', async() => {
        const mc = Mc(service)
        const res = await mc.role_access.select({roleId: 14})
        const res2 = await mc.access.select({})
        console.log(res2.data.length)
        const accessList = res2.data
        const firstList = accessList.filter(element => element.type === 1)
        console.log(59, firstList)
    },2000)
